...
describe('Database Connection - Integration Tests (Real Database)', () => {
  ...
  describe('getConnectionInfo', () => {
    ...
    it('should have working table access', async () => {
      ...
      if (result.connected) {
        ...
        expect(result.schema_status.users_table_count).toBeGreaterThanOrEqual(0);
        expect(result.schema_status.chapters_table_count).toBeGreaterThanOrEqual(0);
        
        console.log('📋 Schema validation passed - all tables accessible');
      } else {
        fail('Database connection failed - check your .env.local configuration');
      }
    }, 15000);
  });
  ...
  describe('performance tests', () => {
    it('should respond within acceptable time limits', async () => {
      ...
      console.log(`⚡ Average connection time: ${avgTime.toFixed(0)}ms over ${iterations} tests`);
