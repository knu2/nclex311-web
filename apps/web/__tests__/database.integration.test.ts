...
// Check if we have environment variables before attempting to import
const hasEnvVars =
  process.env.NEXT_PUBLIC_SUPABASE_URL &&
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

// Conditionally describe the tests only if environment variables are present
  ...
  () => {
    ...
    beforeAll(async () => {
      if (hasEnvVars) {
        ...
        console.log(
          '⏭️  Skipping integration tests - no Supabase config found'
        );
      }
    });
    ...
    describe('getConnectionInfo', () => {
      it('should return real database information', async () => {
        ...
        if (result.schema_status) {
          ...
          expect(typeof result.schema_status.chapters_table_count).toBe(
            'number'
          );
