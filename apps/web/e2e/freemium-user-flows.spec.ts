...
test.describe('Freemium User Flows', () => {
  ...
  test('Guest user can see all chapters and concepts', async ({ page }) => {
    ...
    await expect(page.getByRole('tab', { name: 'All Concepts' })).toHaveAttribute('aria-selected', 'true');
    
    // Verify both free and premium chapters are visible
    await expect(page.getByText(`Chapter ${testChapters.freeChapter.number}: ${testChapters.freeChapter.title}`)).toBeVisible();
    await expect(page.getByText(`Chapter ${testChapters.premiumChapter.number}: ${testChapters.premiumChapter.title}`)).toBeVisible();
  });

  test('Guest user can view a free concept in chapter 2', async ({ page }) => {
    // Mock the free concept API response
    await page.route(`**/api/concepts/${testChapters.freeChapter.concepts[0].slug}`, async (route) => {
      const mockConcept = {
        id: 'concept-2-1',
        title: testChapters.freeChapter.concepts[0].title,
        slug: testChapters.freeChapter.concepts[0].slug,
        content: 'This is a comprehensive guide to pain assessment in nursing practice.',
        conceptNumber: 1,
        chapter: {
          id: 'chapter-2',
        ...
        questions: [{
          id: 'question-1',
          questionText: 'What is the most important aspect of pain assessment?',
          questionType: 'MULTIPLE_CHOICE',
          explanation: 'Patient self-report is the gold standard for pain assessment.',
          options: [
            { id: 'option-1', text: 'Patient self-report', isCorrect: true, orderIndex: 0 },
            { id: 'option-2', text: 'Vital signs changes', isCorrect: false, orderIndex: 1 },
          ],
        }],
        isPremium: false,
      };
    ...
    const freeChapter = page.locator(`text=Chapter ${testChapters.freeChapter.number}:`).locator('..');
    ...
    await expect(page.getByRole('heading', { name: testChapters.freeChapter.concepts[0].title })).toBeVisible();
  ...
  test('Guest user is blocked by paywall when trying to view premium concept in chapter 5', async ({ page }) => {
    // Mock the premium concept API response with 403 error
    await page.route(`**/api/concepts/${testChapters.premiumChapter.concepts[0].slug}`, async (route) => {
      await route.fulfill({
        status: 403,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    const premiumChapter = page.locator(`text=Chapter ${testChapters.premiumChapter.number}:`).locator('..');
    ...
    await expect(page.getByText(`Chapter ${testChapters.premiumChapter.number} - Premium Content`)).toBeVisible();
    
    // Verify value proposition is shown
    await expect(page.getByText('Access to all 323 NCLEX concepts')).toBeVisible();
    
    // Verify pricing information
    await expect(page.getByText('₱2,999')).toBeVisible();
    
    // Following user flow: Redirect to Subscription Page (via Maya)
    await expect(page.getByRole('button', { name: 'Upgrade Now via Maya' })).toBeVisible();
    
    // Verify trust signal
    await expect(page.getByText('🔒 Secure payment processed by Maya Business')).toBeVisible();
    
    // Test going back to dashboard
    await expect(page.getByRole('button', { name: 'Back to Chapters' })).toBeVisible();
    await page.getByRole('button', { name: 'Back to Chapters' }).click();
    await expect(page.getByText('Welcome back!')).toBeVisible();
  });

  test('UI correctly displays lock icons and premium badges', async ({ page }) => {
    await page.goto('/dashboard');
    await expect(page.getByText('Loading chapters...')).not.toBeVisible();
    ...
    const premiumConcept = page.getByText(testChapters.premiumChapter.concepts[0].title).locator('..');
